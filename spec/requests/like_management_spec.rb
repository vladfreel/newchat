...
RSpec.describe "Like management", :type => :request do
  ...
  it "creates a Like and redirects to the Like's page" do
    get "/categories/" + @category.id.to_s + "/images/" + @image.id.to_s
    expect(response).to render_template(:show)

    post "/categories/" + @category.id.to_s + "/images/" + @image.id.to_s + "/likes", params:{ like: attributes_for(:like, user_id: @user.id), category_id: @category.id, image_id: @image.id }
